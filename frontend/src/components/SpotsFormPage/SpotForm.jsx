...
export default function SpotForm({ isNewSpot }) {
  ...
  return (
    <form onSubmit={getErrors} className="spot-form">
      <div className="headers">{isNewSpot ? "Add a new Restaurant" : "Update restaurant info"}</div>
      <h3>Where&apos;s the restaurant you&apos;ve eaten at located?</h3>
      <p>What&apos;s the location of this restaurant?</p>
      <div className="location">
        <label>
          Country <span className="errors message">{errors.country ? errors.country : ""}</span>
        </label>
      ...
      {errors.price ? <div className="errors message">{errors.price}</div> : null}
      <button type="submit">{isNewSpot ? "Add Restaurant" : "Update Restaurant"}</button>
    </form>
  );
}
